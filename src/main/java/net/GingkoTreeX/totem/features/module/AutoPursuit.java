...
public class AutoPursuit extends ModuleManager {
   public AutoPursuit(){
       super("AutoPursuit", Category.COMBAT, "Auto fly to your target(Only HvH)", GLFW.GLFW_KEY_R);
   }
    @Override
    public void onUpdate() {
        if (this.isEnabled()){
            ...
            if (MinecraftClient.getInstance().player != null) {
                target = AuraUtil.attackNearestEntity(MinecraftClient.getInstance().player, 60);
            }
            if (target != null) {
            pursuit(MinecraftClient.getInstance().player,target);
            }
        }
    }
